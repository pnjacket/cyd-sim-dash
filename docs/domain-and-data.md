...
| `ENTITY-RIGADDRESS` | Learned hardware address of the sim PC | `mac` six octets · `learnedAtMs` monotonic ms when it was last confirmed | **Learned, never configured.** Read from the device's own ARP cache for `pcHost` while the PC is reachable, and persisted so it survives the reboot that happens when the rig powers down. Not part of `ENTITY-DEVICECONFIG`: that record is what the *operator* set, and conflating a learned fact with a configured one would put a value on the configuration page that nobody typed and nobody should edit. Carries no classification tag — a MAC on the operator's own LAN identifies a machine they own, not a person. **Discarded when `pcHost` changes or the configuration is erased** — see `INV-RIGADDRESS-FOLLOWS-HOST`.<br><br>Learned from the **source address of an accepted frame**, not from a resolution of `pcHost`. That is the address which demonstrably answered, and resolving the configured name again would give the same answer on a good day and a stale one otherwise. The consequence, stated rather than left implicit: **any host on the LAN that sends accepted frames teaches the panel its address**, and would become the target of a later wake. That sits inside `SEC-LAN-TRUSTED`, and the capability obtained is sending a Wake-on-LAN packet to a machine on your own network — which anyone who can reach the LAN can already do directly, and which needs no help from this device. Tightening it would mean comparing the sender against a resolved `pcHost`, which trades that for a learning path that fails silently when the name resolves to an address the frames do not come from |
...
| `INV-RIGADDRESS-FOLLOWS-HOST` | Changing `pcHost`, or erasing the configuration, discards the learned address. Nothing else expires it: it is re-learned only from an accepted frame, and a frame from the new host would overwrite it only *after* arriving — which is precisely what cannot happen while the machine you are trying to wake is off | Device, on save and on erase |
