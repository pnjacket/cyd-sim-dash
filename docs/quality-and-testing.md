...
| Q22 | With the panel driving, the sim PC is switched off. The panel reads *Telemetry stopped*, and **after the stale-decay period reads *No signal from the PC*** — and a touch then offers the wake. Asserted end to end, because the defect it guards was invisible at the unit tier: every rung of the ladder was individually right | ladder row 6b, and `CAP-WAKE-RIG` reachability |
...
| Q20 | **Against the real rig**: two touches start it and the panel reaches the driving screen unaided. **Triaged against the firmware first** — the rig's Wake-on-LAN path is independently proven, since the operator already wakes it this way by other means (stated 2026-09-26). That removes the four external causes this row was originally written to warn about, and with them the reason to suspect the rig first. What remains outside this product is narrower: whether a subnet broadcast from the panel's *wireless* interface reaches the rig's wired adapter across the access point | `SUCCESS-WAKE-FROM-PANEL` |
