...
- **Input bindings** — *minimal*. The touchscreen carries exactly one action, `CAP-WAKE-RIG`, and it
  has no binding layer: there is one target, it is the whole screen, and it is live in one link state.
  A binding layer would be scaffolding for a second control that does not exist. DOM hooks remain
  *absent* — the two HTTP screens are served as whole documents and nothing scripts them.
...
| `EVT-WAKE` | Wake-on-LAN magic packet | Device → rig's network adapter · `COMPONENT-NET` · serves `CAP-WAKE-RIG` | Six `0xFF` octets followed by the target's six-octet hardware address repeated sixteen times: 102 octets, no header, no version field and no JSON. Sent as UDP to the subnet broadcast address on port 9 | Pre: `ENTITY-RIGADDRESS` holds an address, per `INV-WAKE-NEEDS-LEARNED-MAC`. Post: **none, and none is observable**. The packet is consumed by a network adapter in a powered-down machine; there is no acknowledgement, no response and no way for the device to learn whether it arrived or had any effect. Unversioned by nature — the format is fixed by the Wake-on-LAN convention and is not ours to version |
