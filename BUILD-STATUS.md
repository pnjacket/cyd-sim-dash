...
| 14 | Link-state screen | full | ◐ | **on-rig, partly** | **Ladder row 6b added 2026-09-26**, and `Q22` passed end to end against the panel: `stale` decays to `unreachable` after a minute of silence, and the learned rig address survives so a touch then offers the wake. Found by the operator watching the glass after switching the PC off — `stale` had no exit and persisted for the whole power session, which silently made `CAP-WAKE-RIG` unavailable in the one case it exists for. Every rung of the ladder was individually correct, which is why nothing caught it.<br><br> `unresolved`, `unreachable`, `stale`, `versionMismatch` and the null-while-driving case all observed on the panel. **The nine icons are built 2026-09-26** — composed from primitives as specified, and the two interpolating lines now carry the title and the version pair, which they previously did not. A touch-paced bring-up sheet showed them at full size; the operator saw some, accepted the set as-is, and declined to work through all nine because reaching the panel costs more than the answer is worth. So **`U8` and `U13` are accepted, not verified** — in particular, whether the five Wi-Fi variants are distinguishable *as a set* from the driving position is still unestablished, and that was the whole reason the question was raised. The sheet is removed; it is one flag to restore |
